...
int parse_line(char* line, int pass_no) {
	...
	if (disp) {
		displacement = malloc(strlen(disp) + 1);
		strcpy(displacement, disp);
		displacement[strlen(disp)] = '\0';
		//printf("disp: %s\n", displacement);
	}
...
int main(int argc, const char* fname) {
	...
	memcpy(ORG, output, program_length);

	void (*entry)() = find_symbol("main") + elf_offset + ORG;
	printf("beginning execution @ main() [%#x]\n", entry);
