...
void testc(uint32_t* stack_start) {
	int i = 0;
	kprintx("Stack Trace: ", (uint32_t) stack_start);
	for(i = (uint32_t)stack_start; i > (uint32_t)stack_start-64; i -= 4)
	{
		uint32_t tmp = *(uint8_t*)i;
		kprintx("", tmp);
	}
	for(;;);
}
